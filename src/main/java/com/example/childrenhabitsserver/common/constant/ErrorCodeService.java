package com.example.childrenhabitsserver.common.constant;

public class ErrorCodeService {
    public static final String UN_AUTH = "AUTH.UN_AUTH";
    public static final String LOGIN_INVALID = "AUTH.LOGIN_INVALID";

    public static final String USER_HAD_EXITS = "USER.USER_HAD_EXITS";
    public static final String USER_HAD_NOT_EXITS = "USER.USER_HAD_NOT_EXITS";
    public static final String USER_ID_NOT_VALID = "USER.USER_ID_NOT_VALID";
    public static final String USER_IS_ROOT_ADMIN = "USER.USER_IS_ROOT_ADMIN";

    public static final String REQUEST_RESET_PASSWORD_INVALID = "COMMON.REQUEST_RESET_PASSWORD_INVALID";

    public static final String ID_HABITS_INVALID= "HABITS.ID_HABITS_INVALID";
    public static final String HABITS_NOT_EXITS= "HABITS.HABITS_NOT_EXITS";

    public static final String USER_HABITS_NOT_EXITS= "USER_HABITS.USER_HABITS_NOT_EXITS";

    public static final String ATTENDANCE_HABITS_IN_VALID= "ATTENDANCE.ATTENDANCE_HABITS_IN_VALID";
    public static final String REQUEST_ATTENDANCE_HABITS_IN_VALID= "ATTENDANCE.REQUEST_ATTENDANCE_HABITS_IN_VALID";
}
